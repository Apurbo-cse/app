                            ...
                            <table id="datatable-buttons" class="table table-striped table-bordered">
                                ...
                                @foreach($cates as $cate)
                                    <tr>
                                        ...
                                        <td>{{$cate->title}}</td>
                                        <td><img src="{{asset($cate->image)}}" alt="category Image" style="width: 110px;"></td>

                                        <td><span class="label {{$cate->status ? 'label-success':'label-warning'}}">{{$cate->status ? 'Active':'Inactive'}}</span></td>
                                        
                                        <td>
                                            <a href="{{route('admin.category-single.edit', $cate->id)}}" class="btn btn-success"><i class="fa fa-edit"></i></a>
                                            
                                            <button class="btn btn-danger" type="button" onclick="deleteCategory({{$cate->id}})">
                                                <i class="fa fa-trash-o"></i>
                                            </button>
                                            <form id="delete_from_{{$cate->id}}" style="display: none" action="{{route('admin.category-single.destroy', $cate->id)}}" method="post">
                                                @csrf
                                                @method('delete')
                                            </form>
                                        </td>
